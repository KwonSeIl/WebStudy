...
<%-- BoardModel model=new BoardModel() 
	...
	연산자
	  ...
	  삼항연산자: 조건?값1:값2 
	  --------------------------------------------------------------> EL
	  JSTL => JSP 안에서 java 코딩을 최소화 (자바 제어문 -> 태그형으로 제작)
	  core
	    <c:set>: 변수 설정
	    ...
	    <c:out>: 화면 출력
	      <c:out value="Hello">
	        => out.println("Hello) ==> JQuery, VueJS.. 자바스크립트에서 주로 사용
	        						   ------ $
	    <c:forEach>: for문
	      <c:forEach var="i" begin="1" end="10" step="1"> => 단점: 증가는 가능, 감소는 불가능
	        => for(int i=1;i<=10;i++)
	        		   - -    -- ---
	        		   | |     |   |
	        		 var  begin end step
	    ...
	    <c:choose>
	      <c:choose>
	        ...
	        <c:when test="조건문"></c:when>
	        <c:when test="조건문"></c:when>
	        <c:when test="조건문"></c:when> else
	      </c:choose>
	    <c:if> ==> XML (닫기가 없는 경우에는 오류 발생)
	      <c:if test="">
	        처리문장
	      </c:if>
	      DTD: 태그, 속성 정의 => XML 단점: 태그에 순서 있음(MyBatis,Spring) => XML 기반
	       => XML 파싱 ==> JAXP, JAXB
	       				  DOM/SAX
	       => JSON 파싱 => Rest
	    <c:forTokens>: StringTokenizer
	  format => Oracle sql 문장에서 처리(거의 사용하지 않음)
	    <fmt:formatDate>
	    <fmt:formatNumber>
	  fn => Model에서 처리(거의 사용하지 않음). 메소드 String
	  ------
	  sql => DAO가 더 편리(ORM)
	  xml => OXM이 더 편리
	  ------ sql, xml => 보안의 문제 발생 (거의 사용하지 않음)
	  
	  <c:forEach>, <c:if>, <c:choose>, <c:redirect> => 대소문자 구분함
--%>
<%
